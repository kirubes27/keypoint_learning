...
def ablation_identity_operator(
    ...
    dim = 2 * num_keypoints
    with torch.no_grad():
        model_copy.operator.linear.weight.copy_(torch.eye(dim, device=device))
        model_copy.operator.linear.bias.zero_()
...
def ablation_random_operator(
    ...
    dim = 2 * num_keypoints
    with torch.no_grad():
        model_copy.operator.linear.weight.copy_(torch.randn(dim, dim, device=device) * 0.1)
        model_copy.operator.linear.bias.copy_(torch.randn(dim, device=device) * 0.1)
