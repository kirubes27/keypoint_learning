...
from diagnostics.summarize_stage_r1_deadzone_gate import (  # noqa: E402
    SEEDS,
    SHAPE_WEIGHT,
    summarize,
)
...
def _write_run(root: Path, seed: int, *, passed: bool, failed: list[int]) -> None:
    run = root / "tiny_overfit" / f"coordinate_standard64_k10_deadzone_seed{seed}"
    ...
    payload = {
        ...
        "metrics": {
            ...
            "r1_probe": {
                ...
                "per_channel_median_dominant_mass_r2": [0.85] * 10,
            },
        },
    }
...
def test_summary_requires_two_passes_and_no_persistent_target(tmp_path: Path) -> None:
...
def test_summary_rejects_persistent_target(tmp_path: Path) -> None:
