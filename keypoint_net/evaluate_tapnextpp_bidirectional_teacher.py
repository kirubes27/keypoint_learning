"""Privileged post-hash evaluation of the TAPNext++ bidirectional teacher."""
...
def evaluate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    with np.load(args.raw_predictions) as raw:
        ...
        direction_difference = np.asarray(raw["forward_reverse_difference_px"], dtype=np.float64)
    ...
    require(forward_prediction.shape == expected_shape, "forward prediction shape differs")
    require(reverse_prediction.shape == expected_shape, "reverse prediction shape differs")
    require(forward_visible.shape == expected_shape[:2], "forward visibility shape differs")
    require(reverse_visible.shape == expected_shape[:2], "reverse visibility shape differs")
    require(
    ...
    forward_visual_path = args.output_dir / "WORST_TAPNEXTPP_256_FORWARD_EVENTS.png"
    reverse_visual_path = args.output_dir / "WORST_TAPNEXTPP_256_REVERSE_EVENTS.png"
    ...
    result = {
        ...
        "visibility_diagnostics": {
            ...
            "forward_invisible_count": int(forward_visible.size - np.sum(forward_visible)),
            "reverse_visible_count": int(np.sum(reverse_visible)),
            "reverse_invisible_count": int(reverse_visible.size - np.sum(reverse_visible)),
        },
    ...
    result_path = args.output_dir / "TAPNEXTPP_256_BIDIRECTIONAL_EVALUATION_RESULT.json"
