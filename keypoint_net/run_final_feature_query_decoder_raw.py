"""Decode frozen final-feature query fields without opening evaluation truth."""
...
from certified_witness_capability import file_record, require
...
SCHEMA_VERSION = "final_feature_query_decoder_raw_receipt.v1"
...
def _maximum_replay_difference(
    ...
    for name in sorted(forward):
        ...
        differences[name] = float(np.max(np.abs(left.astype(np.float64) - right.astype(np.float64))))
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for forbidden in ("sealed_validation_truth", "mask_manifest", "evaluation_result"):
    ...
    require(source_receipt.get("repository_head") == "c6802e2aa13da075c7ad969315461000393d860e", "source score lineage differs")
    ...
    smoke = decode_final_feature_scores(final_scores[:smoke_count])
    smoke_seconds = time.perf_counter() - smoke_start
    require(smoke["local_3x3_prediction_px"].shape == (smoke_count, 10, 2), "smoke shape differs")
    ...
    forward = decode_final_feature_scores(final_scores)
    reverse_native = decode_final_feature_scores(final_scores[::-1])
    ...
    arrays_path = args.output_dir / "RAW_FINAL_FEATURE_QUERY_PREDICTIONS.npz"
    np.savez_compressed(
        arrays_path,
    ...
    receipt_path = args.output_dir / "RAW_FINAL_FEATURE_QUERY_RECEIPT.json"
    receipt = {
        "schema_version": SCHEMA_VERSION,
        "artifact_type": "truth_blind_frozen_final_feature_query_decoder",
...
def main() -> None:
    try:
        ...
        print(f"FINAL_FEATURE_QUERY_RAW_FAILED: {exc}", file=sys.stderr)
        raise
